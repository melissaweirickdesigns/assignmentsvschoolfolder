...
export default function CodeView({ color1, color2, gradientType, gradientDirection, anglePercent }) {
    const generateGradientCSS = (color1, color2, gradientDirection, anglePercent, type) => {
      let gradientCSS;
    
      if (color1 && color2) {
          if (type === 'linear') {
              if (gradientDirection === 'angle') {
                  gradientCSS = `linear-gradient(${anglePercent}deg, ${color1}, ${color2})`;
    ...
    const { gradientCSS, gradientType: type } = generateGradientCSS(color1, color2, gradientDirection, anglePercent, gradientType);
  
    console.log(gradientCSS)
  
    return (
      <div className="results-view code-view">
